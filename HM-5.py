...
In the for loop, we iterate over each item in car_data.items() to obtain
the car name and details.
The details are unpacked into the variables year, engine_volume,
and price.
We then check if the search criteria for year, engine volume, and price are
satisfied using the conditional if statement.
If the conditions are met, additional car
details and the car itself are added to the matching_cars list
...
# 'car': (details[0], details[1], details[2], details[3], details[4])
for car, details in car_data.items():
    ...
    if (year >= search_criteria[0]
        ...
        matching_cars.append((car, details))

matching_cars.sort(key=lambda x: x[1][4])  # Sort by price ascending

# Print up to the first five matching cars
for i in range(5):
    car, details = matching_cars[i]
    print(f"{car}: {details}")
